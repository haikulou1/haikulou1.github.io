...
> **Change** `java-interfaces` · **分支/Commit** `AI/task-DEV-ddccb2af` / `5f5b0ec` · **日期** `2026-07-31` · **审查者** AI
...
| `GlobalExceptionHandler` | `java-interfaces/src/main/java/com/example/demo/GlobalExceptionHandler.java` | 全局异常处理（IllegalArgumentException→400） |
...
|----------|------|----------|----------|------|
| GET /api/helloworld 返回 {"message":"Hello World"} | ✅ | §5.1「helloworld \| GET \| /api/helloworld \| 无 \| {"message":"Hello World"} \| ✅ 已定案」；F2 | `HelloWorldController.java:18-21` | 路径、方法、返回体完全匹配 |

### REQ-2: 冒泡排序接口

| Scenario | 结果 | Spec证据 | 代码证据 | 说明 |
|----------|------|----------|----------|------|
| POST /api/bubble-sort 入参 {"arr":[...]} 出参 {"sorted":[...]} | ✅ | §5.1「冒泡排序 \| POST \| /api/bubble-sort \| {"arr":[3,1,2]} \| {"sorted":[1,2,3]}」；F3-F5 | `BubbleSortController.java:26-34` | 契约对称，clone 后排序不污染入参 |
| 算法须为冒泡排序（相邻两两比较交换），升序，禁用 Arrays.sort | ✅ | §5.3「算法必须为冒泡排序…不得替换为 Arrays.sort」 | `BubbleSortController.java:40-56` | 标准冒泡 + 提前退出优化，仍为冒泡排序 |

### REQ-3: 冒泡排序边界

| Scenario | 结果 | Spec证据 | 代码证据 | 说明 |
|----------|------|----------|----------|------|
| 空数组返回空数组 | ✅ | §5.3「空数组返回空数组」 | `BubbleSortController.java:40-42` | n=0 时循环不执行，原样返回空 |
| 单元素原样返回 | ✅ | §5.3「单元素原样返回」 | `BubbleSortController.java:42` | n=1 时 i<0 不成立，直接返回 |
| arr 为 null → 400 | ✅ | §5.3「arr 为 null → 返回 400 错误」 | `BubbleSortController.java:28-30` + `GlobalExceptionHandler.java:17-20` | 抛 IllegalArgumentException，全局处理器转 400 |

### REQ-4: 第三个接口（快速排序）

| Scenario | 结果 | Spec证据 | 代码证据 | 说明 |
|----------|------|----------|----------|------|
| POST /api/quick-sort 升序排序 | ⚠️ | §3.2 U1 列「快速排序接口」为候选（**未定案**）；§5.1 标「第三个接口 \| TBD \| ❌ 待澄清」 | `QuickSortController.java:28-36` | 实现正确且在候选范围内，但设计文档 U1 未最终定案即进入编码，spec→code 可追溯性存在缺口（P2 流程建议，非功能缺陷） |
...
|------|--------------------------------|
| ⚠️ | **A7** `HelloWorldController.java:19`、`BubbleSortController.java:27`、`QuickSortController.java:29` — 公开 Controller 方法缺少方法级 Javadoc（`@return`/`@param`），仅类级 Javadoc 覆盖。对 demo 可接受，建议补全。 |
| ✅ | A1 源文件格式：所有文件 package 声明正确，UTF-8，无 BOM |
| ✅ | A2 import 顺序：项目包→Spring→JDK，分组合理 |
| ✅ | A3 代码样式：4 空格缩进，一致 |
| ✅ | A4 命名规范：PascalCase 类名、camelCase 方法名，符合规范 |
| ✅ | A5 编码实践：DTO 用标准 getter/setter，无魔法值 |
...
|----|------|------|------|-------------------------------------|
| 可靠性 | `reliability-checklist.md` G1–G17 | ⚠️ | P2 | **G4** `GlobalExceptionHandler.java:17` — 仅处理 `IllegalArgumentException`；`HttpMessageNotReadableException`（JSON 解析失败）等未捕获，返回默认 500 而非 400。**G14** `QuickSortController.java:50-51` — Lomuto 以 `arr[high]` 为基准，对已排序/逆序大数组产生 O(n) 递归深度，极端情况 StackOverflowError。**G16** 全局无日志输出，异常未记录，无可观测性。G1/G2/G3/G5-G13/G15/G17：N/A（无并发/资源/事务/MQ/缓存/定时/灰度等场景）。预扫：`scan-all-rules.sh` 已扫无命中。 |
| 安全 | `security-checklist.md` S1–S10 | ⚠️ | P2 | **S6** `BubbleSortController.java:27`、`QuickSortController.java:29` — `arr` 数组无长度上限校验，超大数组可致 OOM/DoS。S1-S5/S7-S10：N/A（无 SQL/XSS/命令注入/越权/密钥/文件/重定向/CSRF 场景）。 |
...
- **合并建议**：通过（5 项 P2 均为可选改进，不阻塞合并）
- **P0**：无
- **P1**：无
- **P2**：
  1. A7 — 公开 Controller 方法补方法级 Javadoc
  2. G4 — GlobalExceptionHandler 增补 `HttpMessageNotReadableException`→400 处理
  3. G14 — QuickSort 改用随机基准或三数取中，规避最坏递归深度
  4. G16 — 引入日志（SLF4J），异常记录与排障可观测性
  5. S6 — `arr` 增加长度上限校验（如 ≤10000），防 OOM/DoS
- **一句话**：三个 REST 接口功能正确、冒泡/快排算法实现规范、边界处理完善；无 P0/P1 阻塞项，5 项 P2 为健壮性与可观测性改进建议。
...
- [ ] **P2** `A7` `HelloWorldController.java:19` — 为 `helloWorld()`、`bubbleSort()`、`quickSort()` 补充方法级 Javadoc（`@return`）
- [ ] **P2** `G4` `GlobalExceptionHandler.java:17` — 增补 `@ExceptionHandler(HttpMessageNotReadableException.class)` 返回 400
- [ ] **P2** `G14` `QuickSortController.java:51` — 改用随机基准或三数取中选 pivot，规避已排序输入的最坏递归深度
- [ ] **P2** `G16` `GlobalExceptionHandler.java:18` — 引入 SLF4J logger，异常处理时记录 warn 级日志
- [ ] **P2** `S6` `BubbleSortController.java:31` — 对 `arr.length` 增加上限校验（建议 ≤10000），超限返回 400
